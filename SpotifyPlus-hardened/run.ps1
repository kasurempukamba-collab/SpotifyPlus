...
function Get-Link {
    param (
        [Alias("e")]
        [string]$endlink,
        [string]$Owner = "kasurempukamba-collab",
        [string]$Repository = "SpotifyPlus"
    )
...
function Add-SpotifyDefenderExclusions {
    ...
    try {
        ...
        $command = @(
            ...
            '$updatedPaths = @($updatedPreferences.ExclusionPath);'
            '$remainingPaths = @($paths | Where-Object { $_ -notin $updatedPaths });'
            'if ($remainingPaths.Count -eq 0) { $verified = $true; break };'
            'if ($attempt -lt 4) { Start-Sleep -Milliseconds 250 };'
            '}'
...
if (-not $defender_exclusions_off) {
    try {
        ...
        $null = Add-SpotifyDefenderExclusions -Path $defenderExclusionPaths
    }
    catch {
        Write-Warning $lang.DefenderFailed
        Write-Verbose $_.Exception.Message
        ...
        if ($test_bak_spa) {
            ...
            if ($spotify_binary_bak -eq $dll_bak) {
                ...
                if (Test-Path -Path $exe_bak) {
                    ...
                    Write-Warning ("Backup copy {0} not found. Please reinstall Spotify and run SpotifyPlus again" -f $binary_exe_bak)
                    if (-not $no_pause) { Pause }
                    Exit
                }
                ...
                if (Test-Path -Path $chrome_elf_bak) {
                    ...
                    Write-Warning ("Backup copy {0} not found. Please reinstall Spotify and run SpotifyPlus again" -f $binary_chrome_elf_bak)
                    if (-not $no_pause) { Pause }
                    Exit
                }
...
if (-not (Test-Path -LiteralPath $spotify_binary_bak)) {
    ...
    Write-Warning ("Backup copy {0} not found. Please reinstall Spotify and run SpotifyPlus again" -f $name_binary)
    if (-not $no_pause) { Pause }
    Exit
}
